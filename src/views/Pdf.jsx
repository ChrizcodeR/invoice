...
const Pdf = ({ data }) => {
  ...
  const handleMicroserviceResponse = async (response) => {
    // Suponiendo que el microservicio devuelve un archivo ZIP
    const blob = await response.blob();
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'PDFs_Divididos.zip';
  ...
  const sendPDFToMicroservice = async (pdfBytes) => {
    try {
      const response = await axios.post('https://microserviceinvoice-production.up.railway.app/api/upload', pdfBytes, {
        headers: {
          'Content-Type': 'application/pdf',
        },
        responseType: 'blob', // Asegurarse de recibir un blob
      });

      console.log('PDF enviado exitosamente:', response.data);
      await handleMicroserviceResponse(response);
    } catch (error) {
  ...
  const generatePDF = () => {
    if (!data || data.length === 0) {
      alert('No hay datos para generar PDFs.');
      return;
    }
    ...
    const content = data.map((item, index) => {
      ...
      return [
        ...
        { text: "\n", pageBreak: index < data.length - 1 ? 'after' : '' },
      ];
    }).flat();
    ...
    pdfMake.createPdf(documentDefinition).getBuffer((buffer) => {
      ...
      sendPDFToMicroservice(pdfBytes);
    });
  };

  return (
    <div>
      <Button onClick={generatePDF}>Send PDF to Microservice</Button>
    </div>
  );
};
